...
def reason(code, return_code=False):
    ...
    status, result = func_timeout(0.1, func)
    if not status:
        return_val = result
    else:
        if result.is_int():
...
def get_reasoning_result(completion):
    try:
        answer = reason(completion)
        if not (answer == UNSAT or answer == AMBIG):
            answer = float(answer)
    except:
        answer = EXCEPTION
    return answer
